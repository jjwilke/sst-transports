...
typedef enum gni_dev_orb_mmr {
        GNI_DEV_ORB_MMR_FIRST = 0,
        GNI_DEV_ORB_MMR_MAX_PID_CNT,
        GNI_DEV_ORB_MMR_LAST
} gni_dev_orb_mmr_t;
...
uint32_t gni_dev_orb_mmr_valid(gni_dev_orb_mmr_t);
const char * gni_dev_orb_mmr_to_str(gni_dev_orb_mmr_t);
#define GET_DEV_ORB_MMR_VALUE     0xffffffffffffffff
...
/* Revision Compatibility structures */
#define GNI_MAX_CAPABILITY_WORDS      6         /* This value will allow 383 capabilities. */

/*
 * This defines the revision and capability information.
 */
typedef struct gni_revision_info {
    uint64_t                revision_instance;                         /* This value will specifiy the last time this information was changed. */
    int                     nid;                                       /* This information is from this nid. */
    uint32_t                user_major_revision;                       /* This is GNI_MAJOR_REV. */
    uint32_t                user_minor_revision;                       /* This is GNI_MINOR_REV. */
    uint32_t                user_code_revision;                        /* This is GNI_CODE_REV. */
    uint32_t                driver_major_revision;                     /* This is GNI_DRV_MAJOR_REV. */
    uint32_t                driver_minor_revision;                     /* This is GNI_DRV_MINOR_REV. */
    uint32_t                driver_code_revision;                      /* This is GNI_DRV_CODE_REV. */
    uint32_t                maximum_defined_capabilities;              /* This is the number of capabilities defined on the sending node. */
    uint64_t                capabilities[GNI_MAX_CAPABILITY_WORDS];    /* Each capability is a single bit. */
    uint64_t                requirement[GNI_MAX_CAPABILITY_WORDS];     /* The requirement for each capability is a single bit. */
} gni_revision_info_t;

/* DQUEUE structures */
#define GNI_DQUEUE_DEFAULT_ENTRIES	1024
#define GNI_DQUEUE_DEFAULT_MSG_SIZE	192

typedef struct gni_dqueue *gni_dqueue_handle_t;

typedef int (*gni_dqueue_cb_t)(void *context, void *data, uint32_t length, uint32_t pe, uint32_t inst_id);

typedef struct gni_dqueue_in_attr {
        uint32_t entries;
        uint32_t msg_size;
        uint64_t flags;
        uint32_t npeers;
        gni_dqueue_cb_t cb;
        void *context;
} gni_dqueue_in_attr_t;

typedef struct gni_dqueue_out_attr {
        uint32_t         pe;
        uint32_t         id;
        gni_mem_handle_t mdh;
        uint32_t entries;  /* each dqueue must match */
        uint32_t msg_size; /* each dqueue must match */
} gni_dqueue_out_attr_t;
 ...
 *
 * OUT
 * nic_fd_ret   NIC file descriptor returned. The NIC file descriptor is used
 *              to retain and hold open this instance of the NIC.
 *
 * Returns:
 * GNI_RC_SUCCESS - Operation completed successfully.
 * GNI_RC_INVALID_PARAM - One of the input parameters was invalid.
 * GNI_RC_PERMISSION_ERROR - This ptag and cookie combination is not valid.
 *
 * Description:
 ...
 * Parameters:
 * IN
 * nic_fd   NIC file descriptor to close. The NIC file descriptor is used
 *          to release the open instance of the NIC.
 *
 * Returns:
 * GNI_RC_SUCCESS - Operation completed successfully.
 *
...
/**
 * GNI_GetCapabilities - Retrieve the GNI revision information from a node.
 * 
 * Parameters:
 * OUT
 * revision_info  This structure will contain the GNI revision
 *                information obtain from the node.
 *
 * Returns:
 * GNI_RC_SUCCESS       - Operation completed successfully.
 * GNI_RC_INVALID_PARAM - Invalid capability or parameter.
 *
 * Description:
 * Retrieve the revision information from the node.
 **/
gni_return_t
        GNI_GetCapabilities(
                OUT gni_revision_info_t         *revision_info
                );

/**
 * GNI_ValidateCapabilities - Validate the GNI Capabilities in the second supplied revision
 *                            information against the first supplied revision information.
 * 
 * Parameters:
 * IN
 * first_revision_info   This structure will contain the first GNI revision
 *                       information to validate against.
 * 
 * second_revision_info  This structure will contain the second GNI revision
 *                       information to validate.
 *
 * Returns:
 * GNI_RC_SUCCESS        - Operation completed successfully.
 * GNI_RC_ERROR_RESOURCE - There is an incompatible capability.
 *
 * Description:
 * Validate the second revision information against the first revision information.
 * If the revisions are incompatible, then a GNI_RC_ERROR_RESOURCE error
 * will be returned.
 *
 * Notes:
 * For the initial release of this feature, the GNI_ValidateCapabilities() function
 * will return GNI_RC_SUCCESS, because there are no incompatible features or
 * capabilities.  However, for later releases, this function will need to be
 * augmented to validate for incompatible features or capabilities.  So for
 * validation between the initial release and a subsequent release of this feature,
 * the validation will be accomplished on the later release.
 **/
gni_return_t
        GNI_ValidateCapabilities(
                IN  gni_revision_info_t         first_revision_info,
                IN  gni_revision_info_t         second_revision_info
                );
...
/**
 * GNI_SetDeviceOrbMMR - Set device ORB Memory Map Register.
 *
 * Parameters:
 * IN
 * nic_hndl             Handle of a currently open NIC.
 * orb_mmr_type         The ORB MMR type to change.
 * new_orb_mmr_value    The new value to be set into the ORB MMR.
 *
 * OUT
 * old_orb_mmr_value    The previous value of the ORB MMR.
 *
 * Returns:
 * GNI_RC_SUCCESS - Operation completed successfully.
 * GNI_RC_PERMISSION_ERROR - the caller of this API does not permission to use it.
 * GNI_RC_INVALID_PARAM - Invalid parameter.
 * GNI_RC_ERROR_RESOURCE - The device ORB MMR is not supported on this device.
 *
 * Description:
 *
 * Sets the device ORB MMR 'orb_mmr_type' to 'orb_mmr_value' for this node.
 **/
gni_return_t
	GNI_SetDeviceOrbMMR(
		IN gni_nic_handle_t      nic_hndl,
		IN gni_dev_orb_mmr_t     orb_mmr_type,
		IN uint64_t              new_orb_mmr_value,
		OUT uint64_t             *old_orb_mmr_value
		);
 ...
 * device_id    The device identifier , e.g. /dev/kgni1 has
 *              device_id = DEVICE_MINOR_NUMBER - GEMINI_BASE_MINOR_NUMBER = 1.
 * job_id       Job container identifier. On systems without job support, this value will
 *              be the user ID.
 * ptag         Protection tag to be used by all applications in the given job container.
 * cookie       Unique identifier. Assigned to all applications within the
 *              job container along with ptag.
...
/**
 * GNI_ConfigureJobFd - Configure parameters of the job and return file descriptor
 *
 * The parameters, return codes and description are identical to GNI_ConfigureJob() with
 * the addition of returning of the nic file descriptor.
 *
 * OUT
 * nic_fd       The file descriptor that is returned from the GNII_DevOpen() call.
 *
 **/
gni_return_t
        GNI_ConfigureJobFd(
